    ...
    def __getitem__(self, idx):
        ...
        if idx % 2 == 0:
            ...
            stego_path = os.path.join(self.stego_dir,
                                    '.'.join(self.cover_lis[cover_idx].
                                            split('.')[:-1]) + '.mat')
            stego_map = io.loadmat(stego_path)['pChange']
            rand_arr = np.random.rand(cover.shape[0], cover.shape[1])
            images = np.copy(cover)
            inf_map = rand_arr < (stego_map / 2.)
            images[np.logical_and(cover != 255, inf_map)] += 1
            inf_map[:, :] = rand_arr > 1 - (stego_map / 2.)
...
class DatasetPair(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.embedding_otf:
            ...
            beta_path = os.path.join(self.stego_dir, '.'.join(self.cover_list[idx].
                                            split('.')[-1]) + '.mat')
...
class RandomBalancedSampler(Sampler):
    def __init__(self, data_source):
        super(RandomBalancedSampler,self).__init__(data_source)
        self.data_source = data_source

    def __iter__(self):
        cover_perm = [x * 2 for x in torch.randperm(len(self.data_source)
                                                    // 2).long()]
        stego_perm = [x * 2 for x in torch.randperm(len(self.data_source)
                                                    // 2).long()]
        return iter(it.next() for it in itertools.cycle(
            [iter(cover_perm), iter(stego_perm)]))
...
class DataLoaderIterWithReshape(_DataLoaderIter):
    def next(self):
        if self.num_workers == 0:
            ...
            batch = self._reshape(self.collate_fn([self.dataset[i]
                                                for i in indices]))
            if self.pin_memory:
...
class DataLoaderStego(DataLoader):
    def __init__(self, cover_dir, stego_dir, embedding_otf=False,
        ...
        super(DataLoaderStego, self).__init__(
            dataset, _batch_size, None, sampler, None, num_workers,
